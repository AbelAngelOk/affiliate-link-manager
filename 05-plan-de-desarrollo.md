...
- ~~**Cambiar contraseña.**~~ **Resuelto el 2026-09-06.** `PATCH /admin/password` (requiere sesión + contraseña actual), pantalla nueva en el dashboard (`/cuenta`, menú "Mi cuenta"). Verificado en vivo: rechaza contraseña actual incorrecta, y tras el cambio el login viejo deja de funcionar y el nuevo sí.
- ~~**"Olvidé mi contraseña".**~~ **Resuelto el 2026-09-06** (proveedor elegido: Resend). `POST /auth/forgot-password` genera un token de un solo uso (vence a la hora) y manda un email con el link vía Resend; `POST /auth/reset-password` lo canjea por una contraseña nueva. Nuevas páginas `/forgot-password` y `/reset-password` en el dashboard. Sin `RESEND_API_KEY` configurado, degrada a loguear el link en consola (mismo patrón que Telegram) — el código está listo para producción, pero el email real no sale hasta cargar esa credencial (ver `04-alcance-y-limitaciones.md`). Verificado en vivo con el log de consola: token inválido/vencido/reusado rechazados, token válido cambia la contraseña.
...
- ~~**Nuevas garantías de calidad para imágenes de producto.**~~ **Resuelto el 2026-09-06, ampliado a múltiples imágenes por producto.** Se implementó más de lo pedido originalmente: no solo reglas de calidad, sino la posibilidad de cargar varias imágenes por producto en tres conjuntos opcionales por proporción (1:1, 2:3, 4:5) — ver `01-solucion-final.md` §2.3 para el detalle completo (tabla `ProductImage`, validación real contra el archivo con `image-size`, `imagenes` agrupado en `GET /v1/products`) y `04-alcance-y-limitaciones.md` para las limitaciones conocidas (valores de mínimo/tolerancia elegidos por criterio, sin reordenar todavía). `imagen_url` (portada) no se tocó, sigue funcionando igual para las apps ya integradas.
- ~~**Entidad "tipo de producto" (campos dinámicos por tipo).**~~ **Resuelto el 2026-09-06**, con el diseño confirmado antes de implementar (campos dinámicos definibles por el usuario desde el dashboard, no fijos por código). Tablas `ProductType`/`ProductTypeField`/`ProductFieldValue`, endpoints CRUD bajo `/admin/product-types`, pantalla `/tipos` para crear tipos y sus campos, y una sección nueva en el detalle de cada producto para asignar el tipo y cargar sus valores. Detalle completo en `01-solucion-final.md` §2.4 y limitaciones conocidas en `04-alcance-y-limitaciones.md` (solo texto/texto largo, un tipo por producto, no expuesto en `/v1/products` todavía). Verificado en vivo: borrar un tipo no borra el producto (solo desasigna), campo obligatorio sin valor rechaza el guardado con `422`.
