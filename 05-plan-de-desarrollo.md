...
- **Cambiar y recuperar contraseña.** Hoy `POST /auth/register`/`login` no tienen contraparte de "olvidé mi contraseña" ni de "cambiar contraseña" estando logueado (ver limitación ya documentada en `04-alcance-y-limitaciones.md` §"Multi-tenant"). Hace falta al menos `PATCH /auth/password` (autenticado, contraseña actual + nueva) y, para recuperación real, un flujo con envío de email (requiere elegir un proveedor de envío de mails, no está resuelto en el stack actual).
- **Bug: `POST /apps` devuelve 500 en producción.** Reportado desde el dashboard (`https://dashboard-peach-sigma-13.vercel.app/apps`, Server Action `validateApp` en `apps/dashboard/app/(admin)/apps/actions.ts`, que llama `POST /admin/apps/{nombre}/check`). No investigado todavía — punto de partida: revisar si es un error real del endpoint (post-migración a email+contraseña) o de `apiFetch`/manejo de la respuesta de error en el server action.
