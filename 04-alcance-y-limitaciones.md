...
- Un producto puede asociarse a más de una app (N:N), y consultarse filtrado por app (`?app_id=`).
...
- Dashboard web de CRUD sobre Apps/Products/Slots/SlotLinks, como cliente más de la propia API (no una capa de datos aparte).
...
- Contrato explícito de disponibilidad vía código HTTP: `GET /r/{slot_id}` devuelve `410 Gone` cuando un slot se queda sin ningún link activo, para que la app pueda ocultar ese botón puntual sin tratarlo como error genérico.
...
- **Fallback automático entre proveedores.** El sistema *no* decide por sí solo "si Amazon MX no está, mostrar el botón de Mercado Libre en su lugar" — eso lo resuelve la app consumidora, mostrando/ocultando sus propios botones según qué slots vengan activos. El fallback automático del sistema es únicamente dentro del mismo canal (mismo proveedor+país, entre los `SlotLink` candidatos).
...
- **OAuth2 multi-tenant real.** Se documenta el plan de migración, pero implementarlo (emisión/rotación de tokens, alta de múltiples usuarios) queda deliberadamente fuera de esta versión al confirmarse que hoy hay un solo usuario.
...
### Amazon
- **Cobertura geográfica real**: Amazon Associates solo opera en 20 países. **Argentina no tiene programa propio** — un slot `provider=amazon, country=ar` no es válido; si se quiere monetizar con Amazon desde Argentina, hay que hacerlo vía el programa de EE.UU. (`country=us`), no uno "local".
- **Problema de arranque ("chicken-and-egg")**: la Creators API exige un mínimo de 10 ventas calificadas en 30 días para dar acceso. Una cuenta nueva, sin historial de ventas, **probablemente no va a poder consultar disponibilidad/precio de Amazon vía API desde el día uno**. Hay que planificar un período inicial donde:
  - la verificación de los `SlotLink` de Amazon es manual (revisión periódica del usuario), o
...
- **Regla de cloaking**: el endpoint `/r/{slot_id}` es válido, pero cada botón de la app tiene que dejar claro antes del click que el destino es Amazon (texto, ícono) — no puede ser un link genérico sin contexto.
...
- **Declaración de sitios**: cada app nueva que use links de Amazon debe declararse manualmente en la cuenta de Associates — no es automatizable, es un paso de onboarding por app, independiente de que la app ya esté modelada como `App` en este sistema.

### Mercado Libre
- **No hay API oficial para generar links de afiliado.** La creación de cada `SlotLink` de tipo ML siempre requiere un paso manual en el panel de afiliados — el sistema no puede "descubrir y linkear" productos de ML por sí solo, solo puede validar lo que ya fue cargado a mano.
...
- **Restricción de tipo de link**: solo product pages o listas de afiliados — coincide con el diseño (cada `SlotLink` es un producto puntual), pero es una restricción real que hay que respetar al cargar links.
...
### Single-tenant (v1) y su migración futura
- **La API key única es, en sí misma, el mayor punto de exposición de esta versión**: cualquiera que la obtenga tiene acceso total (no hay entornos separados que limiten el daño, porque no hay más de un entorno). Hay que tratarla como un secreto real (variable de entorno, nunca en el código del cliente/app pública) — más crítico todavía que en un esquema multi-tenant, donde una fuga de credenciales de un usuario no compromete a los demás.
- **El día que se migre a OAuth multi-tenant, el riesgo pasa a ser de aplicación, no de infraestructura**: cada endpoint va a tener que aplicar el filtro `WHERE owner_user_id = token.sub` de forma consistente — un único endpoint donde se olvide ese filtro es una fuga de datos entre entornos. Vale la pena tenerlo presente desde ahora al escribir el código, aunque hoy solo exista un `owner_user_id` posible, para que la migración no requiera auditar todo el código existente.
...
### Generales
- **Falsos positivos en la detección de "no disponible"**: una caída temporal de la plataforma o un error transitorio de red no debe tirar abajo un `SlotLink` válido. Hace falta un umbral de fallos consecutivos antes de marcarlo `broken` (contemplado en el diseño, pero es un punto de cuidado en la implementación).
- **El redirect no valida en tiempo real**: `GET /r/{slot_id}` confía en el último estado calculado por el job periódico, no verifica el link en el momento del click (por velocidad y para no generar tráfico extra hacia Amazon/ML por cada click real). Esto significa que puede haber una ventana de hasta el intervalo del job (12–24h) donde un link recién roto todavía se sirva como si estuviera activo.
- **Single point of failure**: al mover el click-through a un dominio propio (`/r/{slot_id}`), ese servicio se vuelve crítico para la monetización de *todas* tus apps a la vez. Downtime del servicio de redirección = todos los botones de compra rotos simultáneamente, aunque los links originales estén perfectos. Requiere monitoreo básico de uptime propio.
