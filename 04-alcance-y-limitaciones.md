...
- **Cambiar y recuperar contraseña ya están resueltos** (`PATCH /admin/password` con la actual; `POST /auth/forgot-password`/`reset-password` sin ella, por email vía Resend — ver `01-solucion-final.md` §3). Mientras `RESEND_API_KEY` no esté configurado en producción, el link de recuperación solo se loguea en consola del servidor — nadie externo puede verlo, así que en la práctica "olvidé mi contraseña" no funciona todavía para nadie salvo quien tenga acceso a esos logs.
- **El link de recuperación no invalida sesiones existentes.** Si alguien roba el acceso completo a una cuenta (contraseña + sesión activa), cambiarla por recuperación no cierra la sesión que el atacante ya tenía — mismo comportamiento documentado para `PATCH /admin/password` (ver dos puntos abajo).
...
### Tipos de producto (campos dinámicos)
- **Solo dos formatos de campo** (texto de una línea o texto largo) — no hay número, fecha, selector de opciones, ni referencia a otro producto. Alcanza para el caso real que motivó esto ("autor", "nota del propietario"), pero cualquier tipo que necesite un campo estructurado (ej. "año de publicación" como número validado) hoy lo tiene que cargar como texto libre.
- **Un producto solo puede tener un tipo a la vez** — no hay "libro y también electrónica" combinados.
- **Los valores de campo no se exponen en `GET /v1/products` todavía** (ver `01-solucion-final.md` §2.4) — son datos de gestión interna hasta que haya un caso real que pida mostrarlos a una app consumidora.
- **Borrar un campo borra sus valores en cualquier producto que los tuviera cargados**, sin aviso ni confirmación extra más allá del botón "Borrar campo" del panel — no hay una papelera ni forma de deshacerlo.
