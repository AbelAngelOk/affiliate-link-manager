...
- El modelo de datos ya incluye `owner_user_id` en `App`/`Product` apuntando a una única fila `User`, para que el día que haga falta multi-tenant real el cambio quede acotado a la capa de auth (reemplazar el hook por validación de JWT OAuth) sin tocar el esquema.
...
Aunque el uso principal es programático, tiene sentido un frontend chico para dar de alta/editar Apps, Products, Slots y SlotLinks sin tener que armar requests a mano en Postman para cada cambio:
...
- Vista prioritaria: productos/slots "en alerta" (sin ningún `SlotLink` activo) — es la pantalla que más se va a usar en el día a día, según el flujo descrito en `01-solucion-final.md`.
- No hace falta un framework de admin (Directus/PocketBase, etc.) — al ser CRUD sobre 4-5 entidades y un solo usuario, un puñado de páginas React con formularios alcanza y evita sumar una pieza más al stack.
